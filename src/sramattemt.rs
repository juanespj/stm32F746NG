...
fn framebuffer() -> &'static mut [u16; FB_SIZE] {
    unsafe { &mut *(SDRAM_BASE as *mut [u16; FB_SIZE]) }
}
// ---------------------------------------------------------------------------
// Interrupt binding
// ---------------------------------------------------------------------------
...
fn init_sdram() {
    ...
    // Enable FMC clock
    rcc.ahb3enr()
        .modify(|w: &mut pac::rcc::regs::Ahb3enr| w.set_fmcen(true));
    ...
    // Bank 1
    // ---------------------------------------------------------------------
    fmc.sdcr(0).write(|w: &mut pac::fmc::regs::Sdcr| {
        w.set_nc(0b01.into()); // 8 column bits
        w.set_nr(0b01.into()); // 12 row bits
        w.set_mwid(0b01.into()); // 16-bit bus
        w.set_nb(0b01.into()); // 4 banks
        w.set_cas(0b11.into()); // CAS latency = 3
        w.set_wp(false);
        w.set_sdclk(0b10.into()); // 2 HCLK cycles
        w.set_rburst(true);
        w.set_rpipe(0b01.into()); // 1 HCLK delay
    });

    // ---------------------------------------------------------------------
    // SDRAM Timing Register (SDTR)
    // ---------------------------------------------------------------------
    fmc.sdtr(0).write(|w: &mut pac::fmc::regs::Sdtr| {
        w.set_tmrd(2.into());
        w.set_txsr(7.into());
        w.set_tras(4.into());
        w.set_trc(7.into());
    ...
    // 1. Clock enable
    fmc.sdcmr().write(|w: &mut pac::fmc::regs::Sdcmr| {
        w.set_mode(0b001.into());
        w.set_ctb1(true);
    });
    ...
    // 2. Precharge all
    fmc.sdcmr().write(|w: &mut pac::fmc::regs::Sdcmr| {
        w.set_mode(0b010.into());
        w.set_ctb1(true);
    });
    while fmc.sdsr().read().busy() {}

    // 3. Auto-refresh (8 cycles)
    fmc.sdcmr().write(|w: &mut pac::fmc::regs::Sdcmr| {
        w.set_mode(0b011.into());
        w.set_ctb1(true);
        w.set_nrfs(8 - 1);
    });
    while fmc.sdsr().read().busy() {}
    ...
    let mode: u32 = (0b011 << 0) | // burst length = 8
        (0b0 << 3) |   // sequential
        (0b011 << 4) | // CAS = 3
        (0b0 << 7) |   // standard mode
        (0b1 << 9); // write burst single

    fmc.sdcmr().write(|w: &mut pac::fmc::regs::Sdcmr| {
        w.set_mode(0b100.into());
        w.set_ctb1(true);
        w.set_mrd(mode as u16);
    });
    while fmc.sdsr().read().busy() {}

    // 5. Set refresh rate
    fmc.sdrtr().write(|w: &mut pac::fmc::regs::Sdrtr| {
        w.set_count(683); // ~64ms refresh
    });
}
